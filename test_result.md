...
agent_communication:
  ...
  - agent: "main"
    message: |
      GitHub Actions backend lint fix — 2026-06-01.

      User asked to fix failed Production run 26737262890 job 78792968757. Root cause from CI and local reproduction: Backend Lint failed at `ruff check v2` with F821 because `bulk_mark_attendance.py` raised `ConflictAttendanceExists` without importing it.

      Change: imported `ConflictAttendanceExists` from the coaching domain errors module in `BulkMarkAttendance`.

      Verification passed in the PR worktree using the root backend virtualenv: full `ruff check v2` passed; `ruff format --check v2` reported 473 files already formatted; focused `pytest v2/tests/interface/test_coach_bulk_attendance.py v2/tests/application/test_mark_attendance.py -q` returned 14 passed; `git diff --check` passed. `mypy --config-file pyproject.toml v2` remains blocked before useful checking by the existing duplicate module mapping for `admin_payment_ops.py`.
