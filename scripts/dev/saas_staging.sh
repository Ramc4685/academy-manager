...
ensure_env_file() {
  ...
  # Backfill keys added after the env file was first generated. The proxy
  # shared secret gates the internal tenant header (issue #519): without it
  # the header source is disabled and internal jobs/smokes would 4xx.
  if ! grep -q '^V2_PROXY_SHARED_SECRET=' "${ENV_FILE}"; then
    log "Adding V2_PROXY_SHARED_SECRET to ${ENV_FILE}"
    upsert_env_value V2_PROXY_SHARED_SECRET "$(random_hex 32)"
  fi
}
...
cmd_smoke() {
  ...
  # The internal tenant header is secret-gated (issue #519): the smoke must
  # also present x-cm-proxy-auth with the staging proxy shared secret.
  local proxy_secret=""
  if [[ -f "${ENV_FILE}" ]]; then
    proxy_secret="$(awk -F= '$1 == "V2_PROXY_SHARED_SECRET" { print substr($0, index($0, "=") + 1) }' "${ENV_FILE}")"
  fi
