...
# Host-port override for mongo, generated by ensure_mongo_port_override() when
# the default 27017 is taken by a host mongod. Layered last so its `!override`
# on the ports list wins. Absent on hosts with a free 27017.
MONGO_PORT_OVERRIDE_FILE="${LOCAL_DIR}/mongo-port-override.yml"
MONGO_HOST_PORT_DEFAULT="27017"
MONGO_HOST_PORT_FALLBACK="27018"
...
if [[ -f "${MONGO_PORT_OVERRIDE_FILE}" ]]; then
  COMPOSE_FILES+=(-f "${MONGO_PORT_OVERRIDE_FILE}")
  COMPOSE_DEV_FILES+=(-f "${MONGO_PORT_OVERRIDE_FILE}")
fi
...
# Print the names of NON-Docker processes listening on a port, or nothing.
# Two traps this used to fall into, both of which made every occupied port look
# like a conflict (including ports held by our own already-running stack):
#   1. `lsof` prints a "COMMAND PID USER ..." header that matches no filter, so
#      a plain `grep -v docker` over the raw output is always non-empty.
#   2. `lsof` truncates COMMAND to 9 characters, so Docker Desktop's
#      `com.docker.backend` shows up as `com.docke` and never matches "docker".
# Use -F to get parseable, header-free output and match the truncated name.
port_holder() {
  local port="$1" names
  command -v lsof >/dev/null 2>&1 || return 1
  names="$(lsof -nP -iTCP:"${port}" -sTCP:LISTEN -Fc 2>/dev/null \
    | sed -n 's/^c//p' \
    | grep -vE '^(com\.docke|com\.docker|docker|dockerd|containerd|vpnkit)' \
    | sort -u || true)"
  [[ -n "${names}" ]] || return 1
  printf '%s\n' "${names}" | tr '\n' ' '
}

# Mongo's host port is the one thing that routinely collides, because a locally
# installed mongod owns 27017. Rather than asking the operator to stop their
# database, generate a compose override that binds the container to 27018 on
# the host instead. Idempotent; re-checked on every `up`.
ensure_mongo_port_override() {
  local holder
  if [[ -f "${MONGO_PORT_OVERRIDE_FILE}" ]]; then
    return 0
  fi
  holder="$(port_holder "${MONGO_HOST_PORT_DEFAULT}" || true)"
  [[ -n "${holder}" ]] || return 0

  log "Host port ${MONGO_HOST_PORT_DEFAULT} is held by: ${holder}"
  log "Binding staging Mongo to ${MONGO_HOST_PORT_FALLBACK} instead (${MONGO_PORT_OVERRIDE_FILE})."
  mkdir -p "${LOCAL_DIR}"
  cat > "${MONGO_PORT_OVERRIDE_FILE}" <<YAML
# Auto-generated by scripts/dev/saas_staging.sh — host ${MONGO_HOST_PORT_DEFAULT}
# was already in use. Delete this file to go back to the default port.
services:
  mongo:
    ports: !override
      - "127.0.0.1:${MONGO_HOST_PORT_FALLBACK}:27017"
YAML
  COMPOSE_FILES+=(-f "${MONGO_PORT_OVERRIDE_FILE}")
  COMPOSE_DEV_FILES+=(-f "${MONGO_PORT_OVERRIDE_FILE}")
  COMPOSE=(docker compose -p "${PROJECT_NAME}" "${COMPOSE_FILES[@]}")
  COMPOSE_DEV=(docker compose -p "${PROJECT_NAME}" "${COMPOSE_DEV_FILES[@]}")
  # 27017 is no longer ours to claim; stop pre-flighting it.
  REQUIRED_PORTS=("${REQUIRED_PORTS[@]/${MONGO_HOST_PORT_DEFAULT}/${MONGO_HOST_PORT_FALLBACK}}")
}
...
preflight() {
  ...
  # Resolve the mongo host port before scanning, so a host mongod on 27017
  # reroutes the stack instead of being reported as an unresolvable conflict.
  ensure_mongo_port_override
