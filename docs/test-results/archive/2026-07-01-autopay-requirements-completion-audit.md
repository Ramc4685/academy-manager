...
| 4 | Delete 2 leftover historical `subscriptions` rows | Added `scripts/dev/cleanup_stale_tuition_subscriptions.py`, a dry-run-first utility that selects only incomplete setup-bookkeeping rows with no Stripe subscription id and requires explicit confirmation for deletion. A read-only production audit on the only Fly app (`courtmastr-academy-api`) reproduced the utility's candidate query/filter and found 0 safe-delete candidates: one live `active` subscription is excluded by status, and one `incomplete` row is excluded because its linked enrollment is missing and therefore unverifiable. | Production evidence supports no safe-delete candidates; no destructive cleanup was performed. The orphan incomplete row remains a manual data-quality finding, not an autopay cleanup candidate. |
...
Implementation requirements visible from code are covered by current Slice H worktree commits through `f0ff1f70`, with an additional read-only production audit for the historical subscriptions-row cleanup item. The production audit found 0 safe-delete candidates under the utility's conservative selection rules, so no destructive cleanup was performed. The superficially stale `incomplete` row is not safe to delete automatically because its linked billing enrollment is missing; keep it as a manual data-quality follow-up unless production owners explicitly approve a different remediation path.
...
- `docs/test-results/archive/2026-07-01-autopay-requirements-completion-audit.md` — archived ledger for the requirement completion audit.
...
- 2026-07-01T16:30:15 main/working: Recorded read-only production audit evidence: reproduced stale subscription cleanup selection on courtmastr-academy-api returned 0 safe-delete candidates, so no destructive cleanup was run.
## Verification
...
- 2026-07-01T16:30:15: Production read-only audit evidence supplied by operator: read-only Fly SSH audit against courtmastr-academy-api reproduced the cleanup script's query and in-memory filter without delete capability. Result: subscriptions collection had 2 rows total and cleanup candidate count 0. Row 01KVR2VE0THX60D0PMZ4DGTBSE is active and excluded by status. Row 01KWAD43CNMSS10KSA0JS663XR is incomplete but excluded because linked enrollment enr_5af78a0c1b68a431378e is absent from student_billing_enrollments. No destructive operation was performed.
- 2026-07-01T16:34:14: Fresh completion gate after production read-only evidence: PYTHONPATH=. python -m pytest backend/v2/tests -q from worktree root -> 1 failed, 1996 passed, 5 warnings; failure is the known cwd-path bootstrap FileNotFoundError. Confirmed from backend/: PYTHONPATH=.. python -m pytest v2/tests/application/test_bootstrap_academy.py::test_bootstrap_source_does_not_reference_default_academy_id -q -> 1 passed.
- 2026-07-01T16:34:14: Fresh static/frontend gates: ruff check backend/v2 -> passed; ruff format --check backend/v2 -> 758 files already formatted; lint-imports --config backend/pyproject.toml -> 4 contracts kept, 0 broken; frontend pnpm typecheck -> passed; frontend pnpm lint -> 0 errors, 5 existing warnings; pnpm exec playwright test e2e/specs/billing-health.spec.ts -> 8 passed.
## Reusable Lessons
