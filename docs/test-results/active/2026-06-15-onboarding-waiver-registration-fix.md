...
- 2026-06-20T21:10:35 main/working: Clean PR branch adds admin waiver detail assignment: detail endpoint exposes status/assigned fields and the detail page can require the active waiver for registration without a stale refetch error banner.
## Verification
...
- None recorded yet.
est_admin_waivers_mongo_repo.py backend/v2/tests/interface/test_admin_waiver_template_management.py backend/v2/tests/application/test_admin_waiver_details.py -q => 10 passed, 1 existing StarletteDeprecationWarning; ruff format --check + ruff check touched backend files => passed; pnpm typecheck => passed; pnpm lint => 0 errors, 5 existing warnings; PLAYWRIGHT_PORT=3122 pnpm exec playwright test e2e/specs/admin-waivers.spec.ts => 6 passed. Screenshots captured at output/screenshots/admin-waiver-detail-before-assign.png and output/screenshots/admin-waiver-detail-after-assign.png.
## Reusable Lessons
