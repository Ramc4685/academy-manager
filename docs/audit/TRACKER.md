...
| UIM6 | Bulk user invite UI | [plans/UIM6-bulk-invite.md](plans/UIM6-bulk-invite.md) | S | UIC1 (same surface) | DONE | #449 |
...
- **UIM5** (found during implementation, 2026-07-27): the plan says the admin move "may create a Stripe invoice". It does not — `MoveStudentSessionType.execute` hardcodes `stripe_invoice_id = None` and never calls the injected Stripe gateway; it saves the enrollment and emits `record_session_type_changed` with the prorated `net_cents`. The UI ships describing that actual behavior. **Open backend questions:** whether the move *should* invoice, and separately that it has no idempotency/replay guard (a retry after a timeout emits a duplicate change event).
