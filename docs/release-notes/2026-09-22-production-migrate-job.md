...
- Production migrations are applied by the deploy pipeline, not on boot. A new **Migrate Production** job in `.github/workflows/production.yml` runs after the Production Approval gate: it builds and pushes the backend image once (`registry.fly.io/courtmastr-academy-api:migrate-<sha12>`), runs `python -m backend.v2.migrations --dry-run` on a throwaway machine using that exact image, and fails the workflow (skipping Deploy Backend) when the dry run exits non-zero or its exit code cannot be read.
