...
- `role` and `roles` in the list and detail views are typed as the full domain role set, which includes `student`. A `users` doc can hold `student` next to `parent` or a staff role (a former student who later registers as a parent), and one such row must not fail the whole list with a validation error. `student` is still rejected as a filter value and as an assignable role.
