...
- Fixed during this gate — the `/admin/students` directory now defaults to the "On the books" (operational) lifecycle filter rather than "Everyone" (deliberate, per #773), which made the pre-existing "shows a truthful empty state" e2e spec assert a stale message on first load. Updated the spec to expect "No students match those filters." under the default filter, then switch to "Everyone" to cover the true zero-students case.
