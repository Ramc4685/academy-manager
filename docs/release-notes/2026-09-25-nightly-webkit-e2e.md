...
- `e2e/specs/coach-today.spec.ts` ("server conflict on a plain mark is applied as a correction") now polls the mock's recorded correction calls instead of reading them once. The button shows the mark before the PATCH reaches the mock, and on webkit-mobile under CI load one read saw an empty list. The expected value is unchanged. The WebKit job fails on any retry-pass, so it flaked this PR's nightly run.
