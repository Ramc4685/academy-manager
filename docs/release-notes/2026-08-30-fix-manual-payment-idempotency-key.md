...
instead of silently replaying the first result. Keyed cache entries carry a
payload fingerprint: reusing a key with different fields is rejected with 422
instead of silently replaying the first submission, and a keyed recording also
stamps the payload-derived fallback key so a later keyless identical repeat
still gets the 409. Both admin UIs mint one `crypto.randomUUID()` key per
payment intent (dialog form state) and hold it across retries, so a resubmit
after a 5xx replays instead of double-recording; editing the form rotates the
key.
