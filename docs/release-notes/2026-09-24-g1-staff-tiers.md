...
## Deploy notes
No migration id (none added). No owner steps are needed to deploy. No env vars. Staff with only the billing or front desk role still cannot open the admin app shell; that comes in a later slice, so today the flag is visible through the API and through the owner's preview.
