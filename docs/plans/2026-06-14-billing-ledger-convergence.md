...
1. **Admin Student Billing workflow is merged but not production-proven.** The
   ...
   is configured, wires a gated Resend invoice-email adapter when email delivery
   is configured, and only marks delivery as sent after email succeeds. It still
   needs manual desktop QA and live-like Stripe webhook and Resend validation.
...
3. **Backend route hardening status:** `remove_invoice_line` now uses the
   ...
   send-invoice Stripe checkout URL, Resend email, and PDF behavior in a live-like
...
- [~] **Use case `SendInvoice`** — route exists ([billing_routes.py:547](backend/v2/interfaces/admin/billing_routes.py:547)). Stripe checkout URL wiring is implemented for configured environments, gated Resend invoice-email composition is implemented, and `delivery_status="sent"` is recorded only after email succeeds; Resend/PDF delivery still requires live-like verification before claiming end-to-end collection.
