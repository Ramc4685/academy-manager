...
/**
 * The family record's Billing tab (People CRM A4): the family billing page
 * (spec 2026-09-05-family-billing) re-homed unchanged. It keeps the
 * `admin-family-billing` test id and every action, dialog and owner gate it
 * had as a page; only the timeline moved to the Timeline tab.
 *
 * Record payment always opens `RecordPaymentDialog` (amount, method, an
 * explicit "Record payment" button); nothing here moves money on one click.
 */
export function BillingTab({ parentId }: { parentId: string }) {
