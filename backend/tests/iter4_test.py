...
class TestAutopay:
    ...
    def test_subscription_checkout_real_enrollment(self, parent):
        ...
        eligible = [
            ...
            and (e.get("session") or {}).get("monthly_price")
        ]
        if not eligible:
        ...
        # 200 expected if Stripe + origin allowlisted; validation failures are acceptable
        # when the local demo DB has stale/deleted enrollments.
