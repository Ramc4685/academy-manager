...
``(academy_id, student_id, milestone_key)`` against the REAL claim
(``digest_claim.claim_digest_send``), mirroring
...
async def test_a_fresh_claim_succeeds_and_can_be_marked_sent(db, with_index: bool) -> None:
    ...
    with tenant_scope(ACADEMY_ID):
        ...
        claim = await repo.try_claim(academy_id=ACADEMY_ID, student_id="stu-0", milestone_key="30")
...
async def test_two_ticks_same_milestone_send_exactly_once(db, with_index: bool) -> None:
    ...
    with tenant_scope(ACADEMY_ID):
        ...
        # A second tick for the SAME milestone (e.g. a late-running job on
        # the next day, before the 60-day milestone is due) must never
        # re-claim a sent row.
...
async def test_a_later_milestone_gets_its_own_claim(db, with_index: bool) -> None:
    ...
    with tenant_scope(ACADEMY_ID):
        ...
        first = await repo.try_claim(academy_id=ACADEMY_ID, student_id="stu-1", milestone_key="30")
        assert first is not None
        await repo.mark_sent(first["send_id"])

        second = await repo.try_claim(academy_id=ACADEMY_ID, student_id="stu-1", milestone_key="60")
