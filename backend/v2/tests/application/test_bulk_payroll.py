...
class _FakePeriodRepo:
    ...
    async def list_for_window(self, *, academy_id: str, period_start: datetime, period_end: datetime):
    ...
    async def find_by_id(self, period_id: str): return None
    async def replace(self, period: PayoutPeriod) -> PayoutPeriod: return period
    async def replace_with_lines(self, period: PayoutPeriod) -> PayoutPeriod: return period
...
class _FakeGenerate:
    ...
    async def execute(self, *, coach_id: str, academy_id: str, period_start: datetime, period_end: datetime):
...
async def test_bulk_recompute_skips_non_draft() -> None:
    ...
    result = await uc.execute(academy_id="a1", period_start=START, period_end=END, actor_id="admin-1")
