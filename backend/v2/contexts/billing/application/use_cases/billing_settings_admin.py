...
``allow_platform_charge_fallback`` is read-only here: it is derived from the
deploy-time house-academy setting (``infrastructure/house_academy.py``), not
toggled by an academy admin. The academy owner used to be able to flip it,
which let a tenant route its charges (and its refund/dispute liability) onto
...
class SetInvoiceScheduleSettings:
    ...
    async def execute(self, cmd: SetInvoiceScheduleCommand) -> InvoiceScheduleResult:
        ...
        # Audit BEFORE the write: the schedule must never change unaudited, and a failed upsert leaves
