...
def compose_admin(
    ...
    async def _describe_payout_occurrences(
        ...
        session_ids = sorted(
            {occurrence_session_id(doc) for doc in occ_docs if occurrence_session_id(doc)}
        )
        titles: dict[str, str] = {}
        if session_ids:
        ...
        return {
            str(doc["occurrence_id"]): {
                ...
                "session_id": occurrence_session_id(doc) or None,
                "session_title": titles.get(occurrence_session_id(doc)) or None,
            }
            for doc in occ_docs
        }
    ...
    def _invoice_email_port() -> InvoiceEmailAdapter | None:
    ...
    financial_report_csv = make_financial_report_csv(db)
    ...
    admin = AdminUseCases(
        ...
        list_monthly_payroll=ListMonthlyPayroll(
            reader=MonthlyCoachOccurrenceReaderAdapter(db["session_occurrences"]),
